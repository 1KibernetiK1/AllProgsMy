...
namespace ProjectKeyboardSpy
{
    class GlobalKeyboardSpy
    {
        private IntPtr IdHook;

        public GlobalKeyboardSpy()
        {
            // 1) загружаем библиотеку
            IntPtr instance = LoadLibrary("user32.dll");
            // 2) вешаем ловушку
            IdHook = 
            SetWindowsHookEx((int)HookType.WH_KEYBOARD_LL, 
        ...
        // делегат для ловушки
        delegate IntPtr HookProc(int code, IntPtr wParam, IntPtr l);

        #region Export Win API
        ...
        private IntPtr LowLevelKeyboardProc(int nCode, IntPtr wParam, IntPtr lParam)
        {
            if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN)
            {
                ...
                Keys ks = (Keys) kbd.vkCode;

                MessageBox.Show(ks.ToString());
            }
