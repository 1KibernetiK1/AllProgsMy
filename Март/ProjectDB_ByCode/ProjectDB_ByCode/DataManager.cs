...
namespace ProjectDB_ByCode
{
    public class DataManager
    {
        ...
        public void AddStudentsGroup(string name, string lastname, int Academic)
        {
            var table = _dataSet.Tables[EnumTables.Student.ToString()];
            DataRow row = table.NewRow();
            row["Firstname"] = name;
            row["Lastname"] = lastname;
            row["AcademicGroupId"] = Academic;
            table.Rows.Add(row);
        }
        ...
        public void ChangeStudentGroup(string id, string name, string lastname, int Academic)
        {
            var table = _dataSet.Tables[EnumTables.Student.ToString()];
            DataRow row = table
                .Rows
                .Cast<DataRow>()
                .Where(r => r.RowState != DataRowState.Deleted)
                .First(r => r["StudentId"].ToString() == id);
        ...
        public void RemoveStudentGroup(string selectedStudentId)
        {
            DataTable table = _dataSet.Tables[EnumTables.Student.ToString()];
            foreach (DataRow row in table.Rows)
            {
                string strGroupId = row["StudentId"].ToString();
